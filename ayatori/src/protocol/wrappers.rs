...
pub(crate) struct WrappedArrayFunction<Id: PartyId, P: Protocol<Id>> {
    #[allow(clippy::type_complexity)]
    function: Arc<dyn Fn(&Id, &P::SharedData, Args) -> Value>,
    name: String,
}
...
impl<Id: PartyId, P: Protocol<Id>> WrappedArrayFunction<Id, P> {
    pub fn new<Ret: Any + Send + Sync>(function: impl 'static + Fn(&Id, &P::SharedData, Args) -> Ret) -> Self {
        let name = core::any::type_name_of_val(&function).to_string();
        let wrapped = Arc::new(move |id: &Id, shared_data: &P::SharedData, args: Args| {
    ...
    pub fn call(&self, id: &Id, shared_data: &P::SharedData, args: Args) -> Value {
