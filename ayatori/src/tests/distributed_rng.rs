...
fn commit_to_value<SP: SessionParameters>(_shared_data: &DistributedRNGShared<SP>, args: Args<SP>) -> u64 {
...
fn gen_output<SP: SessionParameters>(_shared_data: &DistributedRNGShared<SP>, args: Args<SP>) -> u64 {
    let bs = args.get_map::<u64>("b");
    bs.values().cloned().sum()
}

impl<SP: SessionParameters> Protocol<SP> for DistributedRNG {
    type SharedData = DistributedRNGShared<SP>;
    type Output = u64;
    fn build(_my_id: &SP::Verifier, shared_data: &Self::SharedData) -> Node<SP, Self> {
...
fn run_protocol() {
    ...
    let sessions = signers
        .into_iter()
        .map(|signer| Session::<TestSessionParams<BinaryFormat>, DistributedRNG>::new(signer, shared_data.clone()))
        .collect::<Vec<_>>();
